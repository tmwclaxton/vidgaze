...
const nextUpScreen = computed(() => {
    return playerStore.findPlayer(props.item.external_id).endScreenNext !== null;
})
    ...
    <div id="endScreen"
        ...
        <div v-if="nextUpScreen"
            ...
            <div class="  ">
                ...
                <!--nex video-->
                <UpNextVideo v-if="useQueueStore().nextItem" :item="useQueueStore().nextItem"/>

            </div>
            <div id="buttons" class="  flex flex-row gap-x-4 select-none ">

                <div @click="nextUpScreen = null;readyForNextVideo = false;"
        ...
        <div class="absolute top-5 left-5 hidden sm:flex flex-row select-none cursor-pointer">
            <Link :href="route('channel.show', {slug: props.item.creator.slug})">
                <img :src="props.item.creator.avatar_url" class="w-8 h-8 rounded-full"/>
            </Link>
            <p @click="" class="line-clamp-1 ml-2 my-auto text-white text dark:textDark text-lg uppercase font-bold" v-text="props.item.title"/>
        </div>
