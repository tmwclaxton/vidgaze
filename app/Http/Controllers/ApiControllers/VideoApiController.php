...
class VideoApiController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (! empty($selectedVideoPlatforms)) {
            $query->whereIn('preferred_source', array_map('strtolower', $selectedVideoPlatforms));
        }
    ...
    public function getVideosByCategory(Request $request)
    {
        ...
        $anchorVideoId = $request->filled('anchor_video_id') ? (int) $request->input('anchor_video_id') : null;
        ...
        $poolMult = max(2, (int) config('vidgaze.recommendations.watch_next_brand_pool_multiplier', 2));
        ...
        $candidateIds = (clone $query)->orderByDesc('time_published')->limit($fetchCount)->pluck('id')->all();
        if ($candidateIds === []) {
            return response()->json([
                'results' => 0,
                'videos' => new VideoCollection(collect()),
        ...
        $scores = CategoryFeedBrandScores::getScores((int) $category->id, $candidateIds);
        arsort($scores);
        $sortedIds = array_keys($scores);

        $anchor = null;
        if ($anchorVideoId !== null) {
            $anchor = Video::query()->whereKey($anchorVideoId)->with('category')->first();
            if ($anchor !== null && (int) $anchor->category_id !== (int) $category->id) {
                $anchor = null;
            }
        }

        $aiCap = min(48, max($per_page, (int) config('vidgaze.recommendations.watch_next_ai_candidate_cap', 48)));
        if ($anchor !== null
            && (bool) config('services.nanogpt.watch_next_ranking_enabled', true)
        ...
        $videos = Video::query()->whereIn('id', $pickIds)
            ->get()
        ...
        if ($anchor !== null && $videos->count() > $per_page) {
            $pool = $videos->all();
            [$ranked, $_meta] = WatchNextVideoAiRanker::rankVideos($anchor, $pool);
    ...
    private function indexRecommended(
        ...
        $excludeIds = array_values(array_unique(array_filter(array_map('intval', $video_ids), fn (int $id) => $id > 0)));
        ...
        if ($viewerCreatorId !== null) {
            $subscribedCreatorIds = Auth::user()->creator->subscriptions->pluck('id')->all();

            $interestCategoryIds = VideoView::query()
                ->where('video_views.viewer_id', $viewerCreatorId)
                ->where('video_views.created_at', '>=', Carbon::now()->subDays((int) config('vidgaze.recommendations.history_days', 90)))
                ...
                ->orderByDesc('c')
                ->limit((int) config('vidgaze.recommendations.max_interest_categories', 5))
        ...
        $subLimit = (int) config('vidgaze.recommendations.subscribed_pool_limit', 32);
        if ($subscribedCreatorIds !== [] && $creator_id === null) {
            $q = $this->recommendedCandidateQuery($excludeIds, $shorts, $selectedVideoPlatforms, $creator_id);
        ...
        $catLimit = (int) config('vidgaze.recommendations.category_pool_limit', 32);
        ...
        if (count($mergedIds) < $cap && $creator_id === null) {
            $q = $this->recommendedCandidateQuery($excludeIds, $shorts, $selectedVideoPlatforms, $creator_id);
            $q->selectRaw('videos.*, IF(view_count = 0, 0, ((view_count + 0.7) / (impressions_count + 1))) as ctr')
                ->orderBy('ctr', 'desc')
        ...
        $mergedIds = array_slice(array_values(array_unique(array_map('intval', $mergedIds))), 0, $cap);

        $videosArr = $mergedIds === []
            ? []
            : Video::query()->whereIn('id', $mergedIds)->get()->all();

        $videosArr = RecommendedVideoAiRanker::sortHeuristic($videosArr, $subscribedCreatorIds, $interestCategoryIds);
        $profile = RecommendedVideoAiRanker::buildViewerProfile($viewerCreatorId, $subscribedCreatorIds, $interestCategoryIds);
        [$ranked, $_meta] = RecommendedVideoAiRanker::rankVideos($videosArr, $profile, $viewerCreatorId);
        $videos = collect($ranked);

        if ($videos->count() < $per_page && $creator_id === null) {
            $amt = $per_page - $videos->count();
            $fillExclude = array_values(array_unique(array_merge($excludeIds, $videos->pluck('id')->all())));
            $randomVideos = $this->recommendedCandidateQuery($fillExclude, $shorts, $selectedVideoPlatforms, null)
                ->inRandomOrder()
                ->take($amt)
                ->get();
            $videos = $videos->merge($randomVideos);
        ...
        if ($first_video_slug) {
            $first_video = Video::where('slug', $first_video_slug)->first();
            if ($first_video) {
                $videos = $videos->filter(fn ($v) => (int) $v->id !== (int) $first_video->id)->values();
                $videos->prepend($first_video);
            }
        }
    ...
    {
        return ['maravilloso', 'animal', 'nude', 'naked', 'nursery', 'rhymes', 'children', 'cartoon', 'kids', 'finger', 'singing', 'toys', 'babies', 'family', 'baby', 'songs', 'song', 'learn', 'learning', 'educational', 'shopkins', 'shoppies', 'numbers'];
    }

    private function recommendedCandidateQuery(array $excludeIds, bool $shorts, array $selectedVideoPlatforms, ?int $creator_id): Builder
    {
        $query = Video::query()->where('visibility', '=', 'public')->whereNotNull('category_id');
