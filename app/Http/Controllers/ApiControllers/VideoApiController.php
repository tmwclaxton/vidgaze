...
class VideoApiController extends Controller
{
    ...
    public function index(Request $request)
    {
        $request->validate([
            ...
            // comma separated list of video ids; empty string is sent from the homepage infinite scroll
            'video_ids' => ['nullable', 'string', 'regex:/^$|^[0-9]+(,[0-9]+)*$/'],
    ...
    public function getPinnedVideos(Request $request)
    {
        if ($request->input('category_slug') === '') {
            $request->merge(['category_slug' => null]);
        }

        $request->validate([
            'per_page' => 'integer|min:1|max:50',
            //            'page' => 'integer|min:1',
            // Slug resolved in code: missing category rows (unseeded DB) return empty results instead of 422.
            'category_slug' => ['nullable', 'string', 'max:64', 'regex:/^[a-z0-9_-]+$/'],
        ...
        if ($request->filled('category_slug')) {
            $category = Category::where(
                'slug',
                $request->string('category_slug')->toString(),
            )->first();
            if ($category === null) {
                return response()->json([
                    'results' => 0,
